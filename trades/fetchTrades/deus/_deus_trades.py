...
class DeusTrades:
    ...
    def __init__(self, infura_key):
        ...
        amm_addr = self.w3.toChecksumAddress('0xd77700fc3c78d1cb3acb1a9eac891ff59bc7946d')
        ...
        self.amm = self.w3.eth.contract(address=amm_addr, abi=amm_abi)
    ...
    def get_transactions(self, fromBlock, toBlock, limit):
        deus_buy_filter = self.w3.eth.filter({
            'fromBlock': fromBlock,
            'toBlock': toBlock,
            'address': self.w3.toChecksumAddress('0x3b62f3820e0b035cc4ad602dece6d796bc325325'),
            'topics': [
        ...
        deus_sell_filter = self.w3.eth.filter({
            'fromBlock': fromBlock,
            'toBlock': toBlock,
            'address': self.w3.toChecksumAddress('0x3b62f3820e0b035cc4ad602dece6d796bc325325'),
            'topics': [
    ...
    def export_old_tr_data(self, old_trs):
        buy = str(self.amm.get_function_by_name('buy'))
        sell = str(self.amm.get_function_by_name('sell'))
        new_trs = []
        out = []
        for log in old_trs:
            info = {}
            ...
            try:
                inputData = self.amm.decode_function_input(tr['input'])
            ...
            if str(inputData[0]) == buy:
                ...
                continue
            timeStamp = self.w3.eth.getBlock(int(tr['blockNumber']))['timestamp']
    ...
    def export_new_tr_data(self, new_trs):
        out = []
        for log in new_trs:
            info = {}
            tr = self.w3.eth.getTransaction(log['transactionHash'])
    ...
    def get_history(self, fromBlock, toBlock, limit):
        trs = self.get_transactions(fromBlock, toBlock, limit)
        old_method_trs, new_trs_log = self.export_old_tr_data(trs)
        new_method_trs = [] if new_trs_log == [] else self.export_new_tr_data(new_trs_log)
