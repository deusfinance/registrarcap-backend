...
def updateTransactions(contract_address):
    ...
    latest_block = int(requests.get("https://api.etherscan.io/api?module=proxy&action=eth_blockNumber&apikey="+apikey_etherscan).json()['result'],0)
    if(os.path.isdir('./'+contract_address+'/')):
        if os.path.isfile('./'+contract_address+'/transactions.json'):
            ...
            with open('/'+contract_address+'/transactions.json', 'w'): pass
            list_transactions = []
            latest_local_block = 0
            latest_local_timestamp = 0
    else:
    ...
    if(latest_local_block != latest_block):
        ...
        request = "http://api.etherscan.io/api?module=account&action=txlist&address="+                                        str(contract_address)+                                        "&startblock="+                                        str(latest_local_block)+                                        "&endblock="+                                        str(latest_block)+                                        "&sort=asc&apikey="+                                         str(apikey_etherscan)
                    
        
        list_api_transactions = requests.get(request).json()['result']
        list_api_transactions = list(filter(lambda tx: int(tx['timeStamp']) > int(latest_local_timestamp) , list_api_transactions))
        
        list_transactions.extend(list_api_transactions)
        
        if(len(list_api_transactions) > 0):
             if bool_log: print("Added " + str(len(list_api_transactions))+ " new transactions.\n------------------------------------------")
        else:
...
def exportToJSON(dataframe):
    ...
    if(contract_address == "0xD77700fC3C78d1Cb3aCb1a9eAC891ff59bC7946D"):
        response_ethplorer =  requests.get("https://api.ethplorer.io/getTokenInfo/0x3b62f3820e0b035cc4ad602dece6d796bc325325?apiKey="+apikey_ethplorer).json()
    ...
    eth_usd_price = float(requests.get("https://api.etherscan.io/api?module=stats&action=ethprice&apikey=Z38JUQ2M61Z7TWK5EDB1NK783RRXPYBWRJ").json()['result']['ethusd'])
    prices = {'eth' : eth_price,
              'usd': float(eth_usd_price) * float(eth_price)}
    json_data['price'] = prices
    json_data['oneHour'] =  (eth_price / dataframe[dataframe['date'] > (dataframe['date'].max() - pd.Timedelta(hours=1))].head(1)['price'].item()) - 1
    json_data['oneDay'] =  (eth_price / dataframe[dataframe['date'] > (dataframe['date'].max() - pd.Timedelta(hours=24))].head(1)['price'].item()) - 1
    json_data['sevenDays'] = (eth_price / dataframe[dataframe['date'] > (dataframe['date'].max() - pd.Timedelta(hours=(24*7)))].head(1)['price'].item()) - 1
    json_data['oneDayVolume'] = {'eth': dataframe[dataframe['date'] > (dataframe['date'].max() - pd.Timedelta(hours=24))]['value'].sum(),
                                 'usd': dataframe[dataframe['date'] > (dataframe['date'].max() - pd.Timedelta(hours=24))]['value'].sum() * eth_usd_price}
...
def updateTransactionInfos(contract_address):
    ...
    if(os.path.isdir('./'+contract_address+'/')):
        if os.path.isfile('./'+contract_address+'/transaction-infos.json'):
            ...
            with open('./'+contract_address+'/transaction-infos.json', 'w'): pass
            list_transactions_infos = []
            latest_local_timestamp = 0
    else:
        os.makedirs('./'+contract_address+'/')
    ...
    if length_transactions > 0:
        for transaction in list(filter(lambda tx: int(tx['timeStamp']) > latest_local_timestamp , list_transactions)):
            request = "https://api.ethplorer.io/getTxInfo/"+str(transaction['hash'])+"?apiKey="+str(apikey_ethplorer)
        ...
        if bool_log: print("Added " + str(len(list_api_transactionInfos)) +" new transaction infos.\n------------------------------------------")
    else:
...
def updateInternalTransactions(list_transactions):
    ...
    if(os.path.isdir('./'+contract_address+'/')):
        if os.path.isfile('./'+contract_address+'/transaction-internal-infos.json'):
            ...
            with open('./'+contract_address+'/transaction-internal-infos.json', 'w'): pass
            list_transaction_internal_infos = []
            latest_local_timestamp = 0
    else:
        os.makedirs('./'+contract_address+'/')
    ...
    length_transactions = len(list(filter(lambda tx: int(tx['timeStamp']) > int(latest_local_timestamp) , list_transactions)))
    
    if(length_transactions > 0):
        for transaction in list(filter(lambda tx: int(tx['timeStamp']) > int(latest_local_timestamp) , list_transactions)):
            request = "http://api.etherscan.io/api?module=account&action=txlistinternal&txhash="+str(transaction['hash'])+"&apikey="+apikey_etherscan
        ...
        if bool_log: print("Added " + str(length_transactions) +" new internal transactions.\n------------------------------------------")
    else:
        if bool_log: print("Already up to date.\n------------------------------------------")
...
def createDataframe(list_transactions, list_transactions_infos, *args, **kwargs):
    ...
    df_infos.loc[mask==False, 'value(token)'] = df_infos.loc[mask==False, 'operations'].apply(lambda x: float(x[0]['value'])/ 1000000000000000000)
    df_infos['date'] = df_infos['timestamp'].apply(lambda x: datetime.fromtimestamp(x))

    df_transactions = df_transactions[list(set(df_transactions.columns)- set(df_infos.columns)) + ['hash']].merge(df_infos, how='outer', on='hash').reset_index(drop=True)

    df_transactions = df_transactions.loc[df_transactions.astype(str).drop_duplicates(keep='first').index].reset_index(drop=True)
    ...
    if(bool_internal == True):
        ...
        df_internal.loc[:,'value'] = df_internal.loc[:,'result'].apply(lambda x: float(x[0]['value']) / 1000000000000000000 )
        df_transactions.loc[df_transactions['to'] == '0x0000000000000000000000000000000000000000', 'value'] =         df_transactions.loc[df_transactions['to'] == '0x0000000000000000000000000000000000000000']['hash'].map(df_internal[df_internal['hash'].isin(df_transactions.loc[df_transactions['to'] == '0x0000000000000000000000000000000000000000']['hash'])].set_index('hash')['value'])
...
def exportToCSV(dataframe):
    ...
    dataframe[['value', 'value(token)', 'transactionType']].to_csv('./'+contract_address+'/highcharts-export.csv', index=False)
...
def exportToArray(dataframe):
    ...
    dataframe = dataframe.groupby(['timestamp','value', 'value(token)','transactionType','hour'])['price'].mean().reset_index()
